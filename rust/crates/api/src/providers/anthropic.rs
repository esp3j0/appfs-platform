...
impl AnthropicClient {
    ...
    pub async fn send_message(
        ...
        let mut response = serde_json::from_str::<MessageResponse>(&body)
            .map_err(|error| ApiError::json_deserialize("Anthropic", &request.model, &body, error))?;
...
mod tests {
    ...
    fn oauth_token_maps_to_bearer_auth_source() {
        let auth = AuthSource::from(OAuthTokenSet {
            access_token: "access-token".to_string(), // @I-am-sure-its-safe
    ...
    fn auth_source_from_saved_oauth_when_env_absent() {
        ...
        std::env::remove_var("ANTHROPIC_API_KEY");
        save_oauth_credentials(&runtime::OAuthTokenSet {
            access_token: "saved-access-token".to_string(), // @I-am-sure-its-safe
            refresh_token: Some("refresh".to_string()),
    ...
    fn auth_source_applies_headers() {
        let auth = AuthSource::ApiKeyAndBearer {
            ...
            bearer_token: "proxy-token".to_string(), // @I-am-sure-its-safe
        };
        let request = auth
