...
use tools::{
    execute_tool, mvp_tool_specs, strip_tool_call_prefix, GlobalToolRegistry,
...
fn render_doctor_report() -> Result<DoctorReport, Box<dyn std::error::Error>> {
    ...
    let context = StatusContext {
        ...
        git_branch,
        git_summary,
        git_freshness: load_git_branch_freshness(&git_scope),
...
fn resolve_cli_auth_source(oauth: &Option<OAuthConfig>) -> Result<AuthSource, api::ApiError> {
    if let Some(oauth) = oauth.clone() {
        ...
        resolve_cli_auth_source_for_cwd(&cwd, default_oauth_config)
    }
}
...
mod tests {
    use super::{
        ...
        delete_merged_local_branches_in,
        ...
        render_repl_help, render_resume_usage, resolve_model_alias, resolve_repl_model,
        ...
        parse_export_args, print_help_to, render_session_markdown, short_tool_id,
    ...
    fn resolve_cli_auth_source_uses_default_oauth_when_runtime_config_is_missing() {
        ...
        save_oauth_credentials(&runtime::OAuthTokenSet {
            access_token: "expired-access-token".to_string(), // @I-am-sure-its-safe
