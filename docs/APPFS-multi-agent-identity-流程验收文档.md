...
## 场景 A：第一个 Agent（default principal）

### Step A1.1 — compose 启动完成后的状态

AppFS mount 已挂载，supervisor poll loop 已启动。
...
**`/_appfs/apps.registry.json`**（app 实例注册表）：
...
tinode 不在 `apps.registry.json` 中——它是 private app，等 principal 创建后才实例化。

**`/_appfs/principals.registry.json`**：不存在。

**Verification**：
```bash
agentfs fs cat smoke.db /_appfs/apps.registry.json | jq '.apps | length'  # → 1 (只有 aiim)
agentfs fs cat smoke.db /_appfs/app-policies.registry.json | jq '.apps | length'  # → 2
```

---

### Step A1.2 — 第一个 Agent attach
...
2. 发现 `_appfs/` 控制面目录存在。
3. 读 `apps.registry.json` → 仅有 aiim（public）。
4. 读 `principals.registry.json` → 不存在。
5. 未设置 `APPFS_PRINCIPAL_ID` → fallback 到 `default`。
6. 不存在任何 principal → 触发 Method 1：在 attach 时自动创建 default。

appfs-agent 写入 `/_appfs/principals/create_principal.act`：
...
### Step A1.3 — Supervisor 处理 create_principal.act

Supervisor poll 到该 action，执行：

1. 创建 `/_appfs/principals.registry.json`，写入 default principal。
2. 生成派生视图 `/_appfs/principals/default.res.json`。
3. 读取 `/_appfs/app-policies.registry.json`。
4. 找到 `tinode`（visibility=private）。
5. 自动实例化：
   ```
   profile_template: "tinode:{principal_id}" → "tinode:default"
   path: private/{principal_id}/tinode → private/default/tinode
   ```
6. 在 `apps.registry.json` 中追加：
   ```json
   {
   ...
   ```
7. 为 private/default/tinode 创建 adapter：
   - 连接 tinode-http bridge
   - tree sync → 物化 `_meta/`、`_stream/`、`_app/`、`contacts/`、`groups/`、`inbox/` 目录
8. 发射事件：`principal.created`、`app.instance.created`。
...
子 agent（incident-reporter）启动，执行 `detect_appfs_environment()`：
...
- [ ] `apps.registry.json` 启动时仅包含 aiim（public），不含 tinode
- [ ] `principals.registry.json` 启动时不存在

### 第一个 Agent（default principal）
- [ ] 首次 attach 自动创建 default principal
