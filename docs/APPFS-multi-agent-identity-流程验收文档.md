...
Compose 启动
 │
 ├─→ 场景 A: 第一个 Agent 启动（auto default principal）
 │   ├─ A1: principal 自动创建
...
6. 不存在任何 principal → 触发 Method 1：自动创建 default。

Agent 写入 `/_appfs/principals/create_principal.act`：
...
- [ ] `private/default/tinode/` 目录物化（_meta/、_stream/、_app/、contacts/、topics/）
