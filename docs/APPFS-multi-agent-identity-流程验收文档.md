...
本文最初按“第一个 agent 启动时创建 default principal”描述。当前实现收敛为：

1. `appfs compose up` 只负责写入 app policy 和 public app 实例，不会单独把 `default` principal 提前建好。
2. 第一个 appfs-agent attach 时会走 attach ensure：未设置 `APPFS_PRINCIPAL_ID` 时 fallback 到 `default`，然后通过 `create_principal.act` 让 AppFS supervisor 物化 `default` principal 和 `/private/default/tinode`。
3. appfs-agent 的 `/principal create` 和 `/principal fork` 仍用于新增非 default principal。

这意味着：`/private/default/tinode` 的出现与第一个 attach 密切相关，而不是单独的 AppFS 启动副作用。
...
Compose 启动
 │
 ├─→ 场景 A: 第一个 Agent attach（auto default principal）
 │   ├─ A1: principal 在 attach 时自动确保
 │   ├─ A2: private app 在 attach 时自动实例化
 │   └─ A3: 首次业务调用 → 透明凭据创建
 │
...
### Step A1.1 — compose 启动完成后的静态状态（尚未 attach）

AppFS mount 已挂载，policy 和 public app 实例已写入，但 `default` principal 还不会在 compose 阶段提前创建。private `tinode` 的实例化要等第一个 appfs-agent attach 触发 attach ensure 之后才发生。
...
**`/_appfs/apps.registry.json`**（app 实例注册表）此时只包含已物化的 public app 实例：
...
**`/_appfs/principals.registry.json`**：此时可能尚未出现 `default` principal。

**Verification**：
```bash
agentfs fs cat smoke.db /_appfs/apps.registry.json | jq '.apps[].instance_id'  # → "aiim"
agentfs fs cat smoke.db /_appfs/app-policies.registry.json | jq '.apps | length'  # → 2
# Before first attach, principals.registry.json may be absent or may not contain "default".
agentfs fs cat smoke.db /_appfs/principals.registry.json | jq '.principals[].principal_id'  # → no "default" before first attach
```
...
appfs-agent 启动交互会话时，或者自动化 smoke 用 `claw status` / `claw skills`
进入同一个 attach 初始化路径时，会先执行 AppFS attach identity ensure：
...
6. attach ensure 负责在缺失时写入 `/_appfs/principals/create_principal.act`，由 AppFS supervisor 幂等消费并创建 default；随后 appfs-agent 以 default 身份过滤 app、skills 和 events。
...
```bash
APPFS_PRINCIPAL_ID=default claw status
```

这会触发同样的 attach/ensure 流程；因此 smoke 脚本不需要手工追加
`create_principal.act`，也不需要依赖 AppFS 启动时自动创建 principal。

---

### Step A1.3 — Supervisor 自动准备 default private app

当前实现里，`default` 不再依赖手动提前写 `create_principal.act`。AppFS 只负责写入 policy 和 public app；第一个 agent attach 时会通过 attach ensure 触发缺失 principal 的创建，然后由 supervisor 物化该 principal 的 private app：

1. 确保 `/_appfs/principals.registry.json` 存在。
2. 如果 `default` 不存在，attach ensure 会通过 `/_appfs/principals/create_principal.act` 触发创建。
...
8. 为 private/default/tinode 创建 adapter：
   ...
   - tree sync → 物化 `_meta/`、`_stream/`、`_app/`、`contacts/`、`groups/`、`inbox/` 目录
...
agentfs fs ls smoke.db /private/default/tinode                                         # → _meta/ _stream/ _app/ contacts/ groups/ inbox/
...
- [ ] `principals.registry.json` 在第一个 attach ensure 后包含 default
- [ ] `apps.registry.json` compose 启动后包含 aiim（public）；`tinode--default` 在第一个 attach ensure 后才出现
- [ ] `private/default/tinode/` 在第一个 attach ensure 后已物化
...
- [ ] attach ensure 会在缺失时写入 `create_principal.act`
- [ ] 首次 attach 自动确保 default principal
- [ ] `create_principal.act` 被 supervisor 消费
- [ ] `principals.registry.json` 创建，包含 default
- [ ] `principals/default.res.json` 生成（派生视图）
- [ ] supervisor 自动实例化 `private/default/tinode`
- [ ] `apps.registry.json` 新增 `tinode--default` instance
- [ ] `private/default/tinode/` 目录物化（_meta/、_stream/、_app/、contacts/、groups/、inbox/）
- [ ] tree sync / app structure bootstrap 不创建 `tinode:default` 凭据
...
- [ ] principal fork 会清掉父 principal 的 app event cursors 和 wake cursors
- [ ] principal fork 提示文本会明确这是新 principal，不是同一 principal 的 session fork
- [ ] principal fork 给出子进程启动命令，且命令中包含 `APPFS_PRINCIPAL_ID=<child>`
...
- [ ] 上游 Tinode 有两个独立账号
- [ ] 两个 agent 的 `<system-reminder>` 互不干扰
- [ ] 子 agent 的 `/status` 清楚显示自身 principal、attach id、private root
- [ ] principal fork 和 `/session fork` 的行为边界在文档里清楚区分
