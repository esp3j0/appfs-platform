...
export function registerTimelineRoute(app: FastifyInstance, registry: AgentRegistry): void {
  app.get('/api/timeline', async (request) => {
    ...
    return { entries: mergeTimelines(perAgent), interactions, compactionBoundaries } satisfies TimelineResponse;
  });
}

function addAppfsInteractions(
  agentName: string,
  timestamp: number,
  entryId: string,
  events: AppfsEventRecord[],
...
function toCrossAgentInteraction(
  event: AppfsEventRecord,
  agentName: string,
  timestamp: number,
  entryId: string,
): CrossAgentInteraction | undefined {
  if (
...
function extractAppfsEvents(
  ...
  const records: AppfsEventRecord[] = [...structuredRecords];
  ...
  if (sourceReminder) {
    ...
    const alreadyCaptured = records.some(
      record => record.eventType === 'message.received' && record.seq === seq,
    );
...
function extractStructuredInputRouterEvents(
  agentName: string,
  msg: ConversationMessage,
  idScope: string,
): AppfsEventRecord[] {
  const records: AppfsEventRecord[] = [];
...
function parseGenericAppfsEventLine(
  agentName: string,
  msg: ConversationMessage,
  line: string,
  idScope: string,
  index: number,
): AppfsEventRecord | undefined {
