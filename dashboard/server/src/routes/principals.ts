...
interface PrincipalRegistryPrincipal {
  principal_id: string;
  display_name?: string;
  description?: string;
  kind?: string;
...
export function registerPrincipalsRoute(
  ...
  app.get('/api/principals', async () => {
    ...
    let registryData: PrincipalRegistry = { version: 1, principals: [] };
    ...
    const principals = registryData.principals.map(principal => {
      ...
      return {
        ...
        model: active?.model,
        permissionMode: active && 'permissionMode' in active ? (active as any).permissionMode : undefined,
      };
    });
