...
export function registerAppEventOverridesRoute(app, registry) {
...
function discoverStaticAppEvents(dumpDir) {
    const discovered = {};
...
function writeOverrides(root, doc) {
    const file = overridesPath(root);
    fs.mkdirSync(path.dirname(file), { recursive: true });
    fs.writeFileSync(file, `${JSON.stringify(doc, null, 2)}\n`, 'utf-8');
    return doc;
}
function normalizeOverrides(doc) {
    return {
        version: 1,
...
function optionalScopesAreValid(value) {
    if (value === undefined) {
        return true;
    }
    if (!value || typeof value !== 'object' || Array.isArray(value)) {
    ...
    return Object.values(value).every(scope => normalizeScope(scope) !== undefined);
}
