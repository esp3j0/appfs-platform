...
function validateComposeContent(projectRoot, content) {
...
export function registerProjectsRoute(app, projectRegistry, runtimeController, deps = {}) {
    ...
    app.post('/api/projects/:projectId/start', async (request, reply) => {
        const { projectId } = request.params;
        const project = projectRegistry.getProject(projectId);
        if (!project) {
            return reply.status(404).send({ error: 'Project not found' });
    ...
    app.post('/api/projects/:projectId/bootstrap', async (request, reply) => {
        const { projectId } = request.params;
        const project = projectRegistry.getProject(projectId);
        if (!project) {
            return reply.status(404).send({ error: 'Project not found' });
    ...
    app.post('/api/projects/:projectId/stop', async (request, reply) => {
        const { projectId } = request.params;
        const project = projectRegistry.getProject(projectId);
        if (!project) {
            return reply.status(404).send({ error: 'Project not found' });
    ...
    app.get('/api/projects/:projectId/compose', async (request, reply) => {
        const { projectId } = request.params;
        const project = projectRegistry.getProject(projectId);
        if (!project) {
            return reply.status(404).send({ error: 'Project not found' });
    ...
    app.post('/api/projects/:projectId/compose/validate', async (request, reply) => {
        const { projectId } = request.params;
    ...
    app.put('/api/projects/:projectId/compose', async (request, reply) => {
        const { projectId } = request.params;
        const { content } = request.body || {};
        const project = projectRegistry.getProject(projectId);
        if (!project) {
